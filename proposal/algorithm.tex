...
Briefly, these algorithms represent the modules required to implement \uppercase{SnipR} functionality. The Setup step populates the \emph{Snippet Space} at indexing time, by invoking a code clone searching and detection tool. Then, the Learning step builds mappings for all the (original-clone) pairs in the \emph{Snippet Space}. When invoking \uppercase{SnipR}'s Twist, the Matching step first maps the retargeting request to its corresponding optimal mapping and then applies it.    
