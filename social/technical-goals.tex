...
under development, and the software engineering team dynamics shaping its
development, with such fidelity that you could predict emergent programming
misbehavior, catch suspicious code or wrong design choices, before they become
ingrained in the system, and even cause imminent failures. Interpersonal
...
look-ahead software introspection (LIT). LIT is aimed at measuring the quality
and trustability of a software system by extending beyond code and binaries
analysis, and utilizing both the rich social interactions that take place within
software development team (the Social Life of the Software, a phrase we coined
to describe these interactions), and the future development states of the
system, to predict emergent programming misbehavior before they become serious
issues. LIT is activated at runtime, when viewing a source file, or an entire
software project. This offers significant advantages. For one, when the
developer hovers the mouse cursor over the artifact, then the analysis results
are immediately available for display, saving valuable developer time. For
another, these results highlights code issues in design time, which developer
can use to asses the quality and trustability of either the current source code,
or the current software project.  The goal of LIT is to identify whether the
current state of a software system exhibit symptoms that may be indicative of
harmful bugs and/or security flaws.}
...
\begin{itemize}
  ...
  %%% USE THIS PARAGRAPH
  % to
  % explore-and-exploit future development states. By exploring and exploiting such
  % states, we will develop predictive models, and software measurements, to
  % discover hidden insights and relationships representing latent programming
  % misbehavior in a software system, in order to make accurate predictions about
  % the quality and trustability of it.
